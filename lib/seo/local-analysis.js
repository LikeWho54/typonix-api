  ...
  if (uid) {
    ...
    try {
      ...
      if (services.length === 0) {
        ...
        const userSelectedUrls = new Set(userSelectedCompetitors.map(url => url.toLowerCase()));
        const guaranteedCompetitors = [];
        const discoveredCompetitors = [];

        sortedBySimilarity.forEach(item => {
          const itemUrl = (item.url || '').toLowerCase();
          if (userSelectedUrls.has(itemUrl)) {
        ...
        // Force-add any user-selected competitors that didn't make it through scraping
        const guaranteedUrls = new Set(guaranteedCompetitors.map(c => (c.url || '').toLowerCase()));
        for (const url of userSelectedCompetitors) {
          if (!guaranteedUrls.has(url.toLowerCase())) {
            const fromMaps = uniqueBusinesses.find(b => (b.url || '').toLowerCase() === url.toLowerCase());
            guaranteedCompetitors.push(removeUndefined({
              title: fromMaps?.title || 'Unknown',
              url,
              address: fromMaps?.address,
              phone: fromMaps?.phone,
              rating: fromMaps?.rating?.value,
              reviewsCount: fromMaps?.rating?.votes_count,
              category: fromMaps?.category,
              latitude: fromMaps?.latitude,
              longitude: fromMaps?.longitude,
              userSelected: true
            }));
            console.log(`⚠️ Force-added user-selected competitor (not scraped): ${url}`);
          }
        }
        ...
        // Combine: user-selected first, then fill with discovered up to 20 total
        const remainingSlots = Math.max(0, 20 - guaranteedCompetitors.length);
        ...
        topCompetitors = finalCompetitors.map(item => removeUndefined({
          title: item.title || 'Unknown',
          placeId: item.place_id || null,
...
async function generateTargetKeywords(uid) {
  ...
  // Filter out keywords with similarity below 40%
  const MIN_SIMILARITY_THRESHOLD = 0.4;
  const relevantKeywords = allKeywords.filter(kw => kw.similarityToServices >= MIN_SIMILARITY_THRESHOLD);

  console.log(`🔍 Filtered to ${relevantKeywords.length} keywords with similarity >= ${MIN_SIMILARITY_THRESHOLD * 100}%`);
  console.log(`❌ Excluded ${allKeywords.length - relevantKeywords.length} keywords below similarity threshold`);

  if (relevantKeywords.length === 0) {
    console.log('⚠️ No keywords found with sufficient similarity for target keyword generation');
    return [];
  }

  const searchVolumes = relevantKeywords.map(k => k.searchVolume).filter(v => v > 0);
  const minVolume = Math.min(...searchVolumes);
  const maxVolume = Math.max(...searchVolumes);

  const competitions = relevantKeywords.map(k => k.competition).filter(c => c > 0);
  const minCompetition = Math.min(...competitions);
  const maxCompetition = Math.max(...competitions);

  const scoredKeywords = relevantKeywords.map(kw => {
