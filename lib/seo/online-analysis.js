  ...
  if (uid) {
    ...
    try {
      ...
      if (services.length === 0) {
        ...
        const guaranteedDomains = new Set(guaranteedCompetitors.map(c => (c.domain || '').toLowerCase()));
        for (const url of userSelectedCompetitors) {
          try {
            ...
            if (!guaranteedDomains.has(domain)) {
              const fromFiltered = filteredCompetitors.find(c => (c.domain || '').toLowerCase() === domain);
              guaranteedCompetitors.push({
                domain,
                similarity: null,
                metrics: fromFiltered?.metrics || null,
                userSelected: true
              });
              console.log(`⚠️ Force-added user-selected competitor (not scraped): ${domain}`);
            }
          } catch (e) {
            console.warn(`⚠️ Could not parse user-selected competitor URL: ${url}`);
          }
        }
