...
1. **Tone & Style**:
   ...
   - **[Punctuation_Prohibition]**: **NEVER use em dashes (—) or en dashes (–) anywhere in the article.** Do not use them to connect clauses, introduce explanations, or for any other purpose. Use a comma, colon, or rewrite the sentence instead.
...
Start writing now. Output ONLY the markdown blog post about "${selectedTitle.targetKeyword}", no additional commentary.`;
  ...
  const blogContent = await callClaude(blogPrompt, 8000, blogSystemPrompt, 'claude-sonnet-5');
