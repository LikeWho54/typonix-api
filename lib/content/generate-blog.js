...
❹/ Business Context (FILTER FOR KEYWORD RELEVANCE):
- Business: ${selectedPlan.businessName}
- Type: ${selectedPlan.businessTypeIdentifier}
${isLocalBusiness ? `- Location: ${location}` : `- Target Market: ${location}`}
- Services: ${services.length > 0 ? services.filter(s => {
    // This is a comment for filtering - in actual implementation, only mention services relevant to the keyword
    return s;
  }).map(s => s.name || s).join(', ') : 'General business services'}

ONLY mention services that are directly relevant to "${selectedTitle.targetKeyword}".
...
RESEARCH & DATA (FILTER BEFORE USE):
${researchData}
...
5. **Business Integration**:
   - ONLY mention services relevant to "${selectedTitle.targetKeyword}"
